---
title: "Amalik03_FinalHomeworkCode_02"
...
x <- 8
l <- 15
eightplus <- ppois(q = x, lambda = l, lower.tail = FALSE) #Probabilities of successfully being more than 8 calls

eightplus #Reports the probability of hearing more than 8 calls from the Titi monkeys
```
...
AM: According to the help document, I need to do 8:20 to include 9 because setting lower tail to false finds probability[X>x].
I changed my x <- 8:30 to 8 since problem does not ask for a range.
I also removed my barplot since it was not asked for.
...
barplot(probset, names.arg = x, space = 0, xlab = "Number of calls", ylab = "Probability of hearing X calls", main = "Probability Mass Function") #plots the probabilities in prob
...
AM: Altered y axis label. It makes sense statistically, but I understand how it may cause confusion for laypeople.

## What is the probability that she will hear exactly 3 calls in a session?
```{r}
x <- 3
l <- 15
threecalls <- dpois(x = x, lambda = l) #Vector of probabilities of hearing x calls, ranging from x=0 calld to x=30 calls with poisson distribution
threecalls # Probability of hearing 3 calls
...
I appreciated Emily's recommendation to set x to 3 rather than a range(previously known as probset) to just show the necessary probability. I changed all my code for the problems that asked similar questions.
...
```{r}
hist(SimCalls, xlim = c(0, 30), xlab = "Number of Calls", ylab = "Frequency", main = "Simulated Calls Over 104 Sessions") #histogram of SimCall as a freqiency
...
I did it because when ylim was not set, the bar would extend past the y axis so I chose a random number so the axis covered most of the frequencies. They are correct that it may not always be the case and can be more than 30. I removed ylim because I dont know what I should set it to (other than 104), with a mathematical/statistical explanation.
